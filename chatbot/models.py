...
class Conversation(models.Model):
    ...
    session = models.ForeignKey(
        ConversationSession, on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    def __str__(self):
        return (
            f"Conversation from {self.timestamp.strftime('%Y-%m-%d %H:%M')} "
            f"(Session: {self.session.title if self.session else 'N/A'})"
        )
